...
| 概览 | `GET /v1/monitor/overview`（社区动态） |
...
投稿由各部署在 Bot 控制台 **统计与语料 → 社区投稿** 提交（经 Bot 代理写中心），主站只读展示。配置百度审核密钥后创建时自动机审；疑似进 `/admin` 待审。中心运维可在 `/admin`（配置 `GALLERY_ADMIN_SECRET`）通过/拒绝待审，或软隐藏已发布投稿。
